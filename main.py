    ...
    try:
        with GigaChat(credentials=GIGA_AUTH, verify_ssl_certs=False) as giga:
            if account_type == "company":
                prompt = f"""
                Ты — эксперт по анализу отзывов. Ниже представлены тексты отзывов клиентов.
                Определи, к какому типу относится заведение (ресторан, кафе, отель, гостиница, магазин, салон красоты, развлекательный центр и т.п.).
                На основе этого дай 3-6 конкретных рекомендаций, что улучшить. Не пиши про «качество еды», если заведение не связано с общепитом.
                Учитывай специфику: для отелей важны чистота, тишина, работа ресепшн; для магазинов — ассортимент, цены, работа касс; для салонов — квалификация мастеров и т.д.
                ...
                prompt = f"""
                Ты — помощник для людей. Определи для начала, к какому типу относится заведение (ресторан, кафе, отель, гостиница, магазин, салон красоты, развлекательный центр и т.п.)(ресторан, кафе, отель, гостиница, магазин, салон красоты, развлекательный центр, учебное заведение(теникум, колледж, университет и т.п.) На основе следующих отзывов ответь одной фразой: стоит ли обычному человеку посетить это заведение.
...
async def compare_with_competitor(user_id: int, competitor_url: str, db: Session = Depends(get_db)):
    ...
    Первым шагом определи, какой это тип заведения, это может быть кафе, ресторан, учебное заведение, школа, университет, техникум, колледж, поликлиника, и тому подобное.
    ...
    if GIGA_AUTH:
        ...
        ai_rec = "Отечественный GigaChat умер"
